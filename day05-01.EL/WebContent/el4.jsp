...
<body>
<%
	// 이페이지에 진입했을 때, 어트리뷰트로 사용할 리스트를 생성
	...
	// 생성된 목록을 request에 어트리뷰트로 추가
	request.setAttribute("stars", stars);
%>

<h3>EL에서 내장객체인 request에 추가된 어트리뷰
트(목록객체) 추출</h3>
JSP 내장객체 : request <br />
EL 내장객체 : requestScope <br />

내장객체에 .(도트) 연산자로 어트리뷰트 이름을 주어 접근
목록객체의 경우 인덱스를 나타내는 괄호 [숫자]로 각 원소에 접근 <br />
<hr />
<ol>
	<li>
	\${requestScope.stars[0]} = ${requestScope.stars[0]}
	</li>
	
	<li>
	\${requestScope.stars[1]} = ${requestScope.stars[1]}
	</li>
	
	<li>\${stars[2]} = ${stars[2]} <br />
		requestScope 객체명을 생략.
		다른 내장객체(pageScope, sessionScope, applicationScope)
		stars 라는 이름으로 추가된 attribute 가 존재하지 않으므로
		내장객체의 범위 이름을 생략 가능
	</li>
	
	<li>
	\${requestScope.stars[3]} = ${requestScope.stars[3]} <br />
	원소에 접근하는 인덱스를 쌍따옴표로 묶어도 인식 됨.
	</li>
	
	<li>
	\${requestScope["stars"][4]} = ${requestScope["stars"][4]} <br />
	.(도트) 연산자 대신[""] 를 사용해도 접근 가능
	</li>
</ol>
...
<h3>EL에서 내장객체인 requestScope 에 추가된 웹 객체 추출</h3>
.(도트) 연산자로 맵 객체 이름으로 접근 후
[](브래킷) 연산자로 맵의 키값으로 접근하여 맵의 value 를 추출
<ol>
	<li>\${requestScope.starMap["S1"] = ${requestScope.starMap["S1"]}</li>
	<li>\${starMap["S2"] = ${starMap["S2"]}</li>
	<li>\${requestScope.starMap["S3"] = ${requestScope.starMap["S3"]}</li>
