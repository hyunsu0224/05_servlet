...
<c:choose>
	<c:when test="${not empty userId and not empty password}">
		...
		쿠키가 만료되면 자동으로 로그아웃 됩니다 <br />
	</c:when>
	<c:otherwise>
		로그인 해야합니다. <br />
		<a href=<%=application.getContextPath() + "/cookie/login/" %>>로그인 하러 가자</a>
	</c:otherwise>
</c:choose>
<hr />
${applicationScope}
